...
if [ "$skip_debootstrap" != "true" ]; then
  rm -rf $ROOTDIR
  mkdir $ROOTDIR
  debootstrap --arch i386 --no-check-gpg sid $ROOTDIR http://http.debian.net/debian/
fi
...
# Install cool term
$CHROOTCMD apt-get -y --force-yes install zsh
...
# Create a default user "edison" with password "edison"
# Encrypted password is created with mkpasswd
$CHROOTCMD useradd -m edison -p YyleQbUNcJwao
$CHROOTCMD adduser edison sudo
$CHROOTCMD adduser edison netdev
$CHROOTCMD chsh -s /bin/zsh edison
...
if [ "$my_mode" = "devenv" ]; then
  ...
  $CHROOTCMD dpkg -i /tmp/deb/edison/kernel-image-3.10.17-smirnov_3.10.17-r0_i386.deb
  $CHROOTCMD dpkg -i /tmp/deb/edison/kernel-3.10.17-smirnov_3.10.17-r0_i386.deb
  $CHROOTCMD dpkg -i /tmp/deb/edison/kernel-module-bcm4334x_1.141-r47_i386.deb
...
# Load kernel modules
$CHROOTCMD depmod -a 3.10.17-smirnov
$CHROOTCMD ls /home
